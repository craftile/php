...
class JsonViewCompiler extends Compiler implements CompilerInterface
{
    ...
    public function compileJsonFile(string $path): string
    {
        ...
        try {
            ...
            $escapedPath = var_export($path, true);

            return <<<PHP
            <?php \\Craftile\\Laravel\\Facades\\BlockDatastore::loadFile($escapedPath); ?>
            <?php \\Craftile\\Laravel\\Events\\JsonViewLoaded::dispatch($escapedPath); ?>
            $compiledTemplate
            PHP;
        } catch (JsonViewException $e) {
            throw $e;
    ...
    public function compileTemplate(array $templateData, string $path = ''): string
    {
        ...
        foreach ($templateData['regions'] as $region) {
            ...
            <?php if (craftile()->inPreview()) {
                craftile()->startRegion("{$regionId}", "{$regionName}");
